...
# pulling out end year doesn't work because there is no variable if it is "current"
 ...
 out <- lapply(pcodes, function(z) orcid_service(params$orcid.id, put_code=z))
 end_year <- vapply(out, function(w) w[[1]]$`end-date`$year$value, "")
end_year <- append("Current", end_year)
out[[1]]$`7620907`


vapply(out, function(w) w[[1]]$`end-date`, "")

currentfunction <- if(is.null(out[[1]]$`7620907`$`end-date`)){
  out[[1]]$`7620907`$`end-date`$year$value <- "Current"
}
...
membership <- tibble(
  ...
  time = purrr::map_chr(membership, "membership-summary.start-date.year.value"))

detailed_entries(membership, what = title, with = organization, when = paste("Joined", time, sep = ": "))
```

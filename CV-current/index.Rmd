...
conway <- my.orcid("0000-0002-9583-8636")
conwaydf <- conway$papers
impact_factors <- scholar::get_impactfactor(conwaydf$Journal)
conwaydf$impact_factor <- impact_factors$ImpactFactor
conwaydf <- conwaydf %>%
  mutate(impact_factor = replace(impact_factor, impact_factor == 244.585, NA))
      
      if(nrow(conway$authors) > 1){conwaydf$Authors = apply(conway$authors[, 1:12], 1, paste5, collapse=", ")} #
      if(nrow(conway$authors) == 1 ){conwaydf$Authors = paste5(conway$authors, collapse=", ")} #

    # add et al
    if(12 < ncol(conway$authors)){ # don't add if at max author number
      index = conway$authors[, 12+1] != '' # something in next author
      conwaydf$Authors[index] = paste(conwaydf$Authors[index], ", ", 'et al', sep='')
    }

conwaydf$Authors <- str_replace(conwaydf$Authors, "A. Conway", "Aaron Conway")
papers <- nrow(conwaydf)
```
...
>Dr Conway's publication metrics indicate a rising trajectory in terms of the quality and quantity of academic papers he has published. Aaron has published `r renderText(papers)` articles across multiple disciplines including nursing, anesthesia and cardiology. 


<br><br>
```{r }
  conwaydf %>% 
  select(Title, Authors, Journal, Year, impact_factor) %>%
  kable(col.names = c("Title", "Authors", "Journal", "Year", "Impact Factor")) %>%
  kable_styling(bootstrap_options = "hover") %>% 
